-- RECURSIVE QUERY TO GENERATE PROJECT HIERARCHY
...
-- BASE HIERARCHY WITH PATH CONSTRUCTION
BASE_PROJECT_HIERARCHY AS (
  SELECT
    ...
    CASE
      WHEN PH.CONTROL_PROJECT_NAME <> PH6.NAME THEN CONCAT_WS('\', PH.CONTROL_PROJECT_NAME, PH6.NAME, PH5.NAME, PH4.NAME, PH3.NAME, PH2.NAME, PH.NAME)
      ELSE CONCAT_WS(' \ ', PH6.NAME, PH5.NAME, PH4.NAME, PH3.NAME, PH2.NAME, PH.NAME)
    END AS "path"
  FROM
    PROJECT_HIERARCHY PH
    LEFT JOIN PROJECT_HIERARCHY PH2 ON PH.PARENT_PROJECT_ID = PH2.ID AND PH2.LVL = 2
    LEFT JOIN PROJECT_HIERARCHY PH3 ON PH2.PARENT_PROJECT_ID = PH3.ID AND PH3.LVL = 3
    LEFT JOIN PROJECT_HIERARCHY PH4 ON PH3.PARENT_PROJECT_ID = PH4.ID AND PH4.LVL = 4
    LEFT JOIN PROJECT_HIERARCHY PH5 ON PH4.PARENT_PROJECT_ID = PH5.ID AND PH5.LVL = 5
    LEFT JOIN PROJECT_HIERARCHY PH6 ON PH5.PARENT_PROJECT_ID = PH6.ID AND PH6.LVL = 6
  WHERE
    PH.LVL = 1
),
  ...
  FROM
    (
      SELECT
        ...
        ROW_NUMBER() OVER (PARTITION BY HU.SYSTEM_USER_ID ORDER BY MAX(HE.CREATED_AT) DESC) AS RN
      FROM
...
EVENT_SUMMARY AS (
  SELECT
    ...
    COALESCE(PH.PARENT_PROJECT_NAME, ' @' || HP.NAME) AS PARENT_PROJECT,
    PH.PATH AS PROJECT_PATH,
...
--Final output with conditions
SELECT  *  FROM  EVENT_SUMMARY WHERE CREATED_AT >= CURRENT_DATE - interval ' 91 days '   AND HISTORICAL_EVENT_TYPE_ID  in (87,84)
--87 Access Authoring View
--84 is for Access View